...
- **Sandbox path:** Android now uses an official CLI `--patch` overlay to replace only the `ctx.sandbox` provider with a first-party Community adapter. `scripts/android-sandbox-landlock-probe.sh` cross-builds the unmodified `src/main.c` shipped by frozen `@deepseek-ai/node-addon-landlock-run@0.1.1`; the provider accepts only a fully-enforced Landlock probe. adb-shell success is not APK app-UID acceptance.
