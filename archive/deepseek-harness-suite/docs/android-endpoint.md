...
> 事实快照:2026-08-17。状态:`[UNVERIFIED]` —— 通过 Termux 验证前不承诺任何能力。

[English](android-endpoint.en.md) · [Reality Gate](reality-gate.md)

## 定位

Android 是 DSH 社区发行版的**第四个社区端点**(WSL/Linux 终端、Windows 桌面、macOS 桌面之外)。产品形态是 APK:WebView 壳 + `nodejs-mobile` 内嵌官方 `@deepseek-ai/dsh` runtime,延续 Suite 的 Official-Runtime-Centric 薄壳模式,不复实现 Agent loop。
