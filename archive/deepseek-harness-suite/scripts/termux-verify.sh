...
if ! npm ls -g "$DASH_PKG" >/dev/null 2>&1; then
  ...
  npm install -g "$DASH_PKG" 2>&1 | tail -3
fi
...
pass "official runtime installed"
...
dsh web --port "$VERIFY_PORT" >"$HOME/.dsh/termux-dsh.log" 2>&1 &
DSH_PID=$!
trap 'kill $DSH_PID 2>/dev/null || true' EXIT

OK=0
for i in $(seq 1 30); do
  if curl -sS -o /dev/null -w '%{http_code}' "http://127.0.0.1:$VERIFY_PORT/" 2>/dev/null | grep -qE '200|301|302'; then
    OK=1; break
  fi
  sleep 1
done
