...
- adb-shell device mode runs a Koffi `getpid()` FFI smoke, a node-pty `/system/bin/sh` PTY smoke, a `/proc/<pid>/stat` + foreground-PGID + group-SIGINT recovery smoke, and a frozen `sharp@0.35.4` → `@img/sharp-wasm32@0.35.4` 2×2 PNG smoke. Process-control success is explicitly labeled `NODE_PTY_PROC_CONTROL_OK_NOT_APP_UID_ACCEPTANCE`; it is not APK-UID evidence.
...
1. **Terminal / PTY:** the production-shaped Community provider now exists instead of relying on the official test hook. The Android overlay replaces the shipped `id: subprocess` row **in place**. `AndroidSubprocessRuntime extends LocalSubprocessRuntime`, so official `resolveExecutable()`, ordinary `spawn()`, process-tree ownership, collection/spill, and environment scrubbing remain authoritative; only `spawnTerminal()` is overridden. The Android terminal layer uses frozen `node-pty@1.2.0-beta.15` and reads only `/proc/<pid>/stat` for PID/starttime, PPID, PGRP, session, tpgid, and state, with PID-reuse fencing. It deliberately does not read `/proc/<pid>/mem` or task syscalls. When stdin wait cannot be proven it reports `inputWaiting=false`: the official E2B provider uses the same conservative fact, and official terminal-bash still converges through controlled prompt + foreground PGID or `inferred_idle`. The provider also enforces the official `MAX_TIMER_DELAY_MS` grace bound and preserves `scrubbedParentEnv()` plus explicit undefined tombstones. **The APK app-UID preflight is now wired for PTY too**: before official DSH spawn it must allocate node-pty and prove PTY root proc identity, session/PGRP/tpgid/tty facts, foreground-group signal-0 visibility, output delivery, and a node-pty exit event. Wiring is still not device evidence; the release gate is frozen node-pty build/load + real app-UID preflight PASS + a shipped `minimal` persistent-terminal user path, and the official `terminalInspector` test hook is never used.
2. **Sandbox:** this has a production-shaped route. The public `@deepseek-ai/dsh-sandbox` service is the official provider seam. The Android `--patch` overlay replaces the shipped `id: sandbox` provider row in place, preserving its service id and composition order; the official bundle is untouched. The provider derives grants from official `writableRoots(policy)`, speaks only the frozen `@deepseek-ai/node-addon-landlock-run@0.1.1` CLI, preserves exit-125 plus `landlock-run:` failure classification, and advertises `full` only after an exact `landlock: fully enforced` probe. The frozen npm package itself publishes `src/main.c`; `scripts/android-sandbox-landlock-probe.sh` cross-builds that source with the NDK without downloading or patching it. The embedded Node `android-app-uid-preflight.cjs` repeats the full probe **before** official DSH spawn and proves one allowed write plus one denied sibling write under the same APK UID. Wiring alone is still not device evidence.
...
G2 therefore remains **BLOCKED**. Sandbox, hard-link, and PTY-substrate checks are now wired as a fail-closed APK app-UID startup preflight, so a future verified carrier cannot start official DSH unless all three pass. Remaining acceptance work is real Node/NDK/native-addon device evidence, the real app-UID preflight, a shipped `minimal` persistent-terminal user path, sharp device execution, and either an official ripgrep executable-path seam or an upstream Android platform package.
