# 对外叙事 · One Harness. Three Community Endpoints.
...
**我们没有再造 DeepSeek Harness。我们在官方 Harness 外面做一个社区发行版：一套 Runtime，三个社区端（WSL/Linux 终端、Windows 桌面、macOS 桌面），和官方 Web 共用同一套 `~/.dsh`。**

## 三句话版本

1. 官方 DeepSeek Harness 是发动机,社区层不重新实现 Agent loop、Session persistence、Tool execution。
2. 社区只发行三个端：WSL/Linux 上的 `dsh-community`、Windows Setup.exe、macOS dmg。官方 Web 是官方入口，不是我们的端。
...
- 生态定位是 distribution,不是单个桌面壳:三个社区端是发行面,Registry = 验证层,Marketplace = 发现与安装 UX,Handbook = 文档。
- 不要说“我们有 Web + Desktop + Terminal 三端”。Linux AppImage 不是第四个产品端。
