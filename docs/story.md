...
2. 社区基于官方内核，当前发行四个入口：WSL/Linux 上的 `dsh-community`、Windows Setup.exe、macOS dmg、Linux AppImage。Android（Labs）保持 `[UNVERIFIED]` 且不进入 Latest；官方 Web 是内核自带界面，不是我们的发行端。
...
- 生态定位是基于官方内核的社区发行：四个已发行入口是当前发行面；Android 是未验证实验端。Registry = 验证层，Marketplace = 发现与安装 UX，Handbook = 文档。
- 不要说“我们另做了一套 Harness”。官方 Web 是内核自带界面；四个已发行 Community 端基于官方内核，Android 只保留实验状态。
