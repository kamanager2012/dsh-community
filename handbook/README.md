...
如果你维护的是 DeepSeek Harness Community 生态，请先读[社区生态与产品入口](content/00-overview/community-ecosystem.md)和[当前发行状态](content/11-operations/community-release-status.md)：普通用户只下载 `dsh-community`；Suite 已并入本仓 `archive/deepseek-harness-suite/`（原独立仓删除）；独立 Marketplace、独立 Plugins 与 Edition 的内容已并入本仓，原仓于 2026-09-13 删除。冻结 Labs 的历史门禁见 [Community Labs handoff](content/11-operations/community-labs-handoff.md)。
